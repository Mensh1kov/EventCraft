...
@register_tool(
    name="create_vendor",
    description=(
        "Создать/завести ПОДРЯДЧИКА (vendor / contractor) в справочнике подрядчиков воркспейса. "
        "Используй ИМЕННО этот инструмент, когда пользователь просит «заведи/создай/добавь подрядчика», "
        "«добавь фотографа/кейтеринг/ведущего/певца/декоратора в базу». "
        "Подрядчик — это исполнитель (человек/компания), НЕ проект и НЕ задача. "
        "НИКОГДА не создавай для этого проект (create_project) или задачу (create_issue). "
        "Категорию выведи из роли: фотограф→photography, видео→video, кейтеринг→catering, "
        "звук/музыка/певец→sound_lighting, декор→decor, ведущий→mc, транспорт→transport, иначе→other. "
        "Имя бери как назвал пользователь (например, «Фотограф Ваня» или «Ваня»)."
    ),
    input_schema={
        "type": "object",
        "properties": {
